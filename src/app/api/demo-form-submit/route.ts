...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Validate file sizes and types - Reduced limits for Vercel
    const MAX_FILE_SIZE = 10 * 1024 * 1024; // 10MB (Vercel function limit)
    const MAX_VIDEO_SIZE = 25 * 1024 * 1024; // 25MB for videos (special handling)
    ...
    const airtableResponse = await fetch(
      `https://api.airtable.com/v0/${process.env.AIRTABLE_BASE_ID}/Form%20Submissions?typecast=true`,
      {
        method: "POST",
        headers: {
    ...
    if (error instanceof Error) {
      if (
        ...
        errorMessage =
          "Request timed out. Please try with a smaller video file.";
        statusCode = 408;
      } else if (
        error.message.includes("memory") ||
        error.message.includes("MEMORY")
      ) {
        errorMessage = "File too large. Please try with a smaller video file.";
        statusCode = 413;
      } else if (
