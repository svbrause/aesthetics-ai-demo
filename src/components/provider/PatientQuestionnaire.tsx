...
export function PatientQuestionnaire({
  ...
  const handleMultiSelectChange = (
    question: "focusArea" | "aestheticGoal",
    value: string
  ) => {
    setAnswers((prev) => ({
      ...prev,
  ...
  const canProceed = () => {
    switch (currentStep) {
      case 1:
        return (
          answers.focusArea.length > 0 &&
          (!answers.focusArea.includes(
            "Skin (Texture, tone, overall health)"
          ) ||
            (answers.skinConcerns && answers.skinConcerns.length > 0))
        );
      case 2:
        return answers.aestheticGoal.length > 0;
      case 3:
        return (
                      ...
                      {focusAreas.map((area) => (
                        <Card
                          ...
                          className={`p-4 cursor-pointer transition-all duration-200 ${
                            answers.focusArea.includes(area)
                              ? "ring-2 ring-blue-500 bg-blue-50 dark:bg-blue-900/20"
                              : "hover:bg-gray-50 dark:hover:bg-gray-800"
                          }`}
                          onClick={() =>
                            handleMultiSelectChange("focusArea", area)
                          }
                        >
                    ...
                    <div className="space-y-3">
                      {aestheticGoals.map((goal) => (
                        <Card
                          key={goal}
                          className={`p-4 cursor-pointer transition-all duration-200 ${
                            answers.aestheticGoal.includes(goal)
                              ? "ring-2 ring-blue-500 bg-blue-50 dark:bg-blue-900/20"
                              : "hover:bg-gray-50 dark:hover:bg-gray-800"
                          }`}
                          onClick={() =>
                            handleMultiSelectChange("aestheticGoal", goal)
                          }
                        >
                          <div className="flex items-center">
                            <div
