...
export function PatientDetailScreenV2({
  ...
  const calculateOverallScore = () => {
    ...
    // Calculate based on patient's actual finding scores
    const patientFindingScores = patient.findings
      .filter((finding: any) => typeof finding === "object" && finding.score)
      .map((finding: any) => finding.score);

    if (patientFindingScores.length > 0) {
      const averageScore =
        patientFindingScores.reduce(
          (sum: number, score: number) => sum + score,
          0
        ) / patientFindingScores.length;
      return Math.round(averageScore);
    }
  ...
  const interestedAreas = filteredAnalysisAreas
  ...
  const areasToShow = showOnlyInterestedAreas
    ? filteredAnalysisAreas.filter((area) => interestedAreas.includes(area.id))
          ...
          >
            <div className="flex space-x-1 p-1 bg-gray-800/50 rounded-xl border border-gray-700/50">
              {[
                ...
                >
                  {view.icon}
                  <span className="ml-2 font-bold text-base">{view.label}</span>
                </Button>
              ))}
            </div>
          </motion.div>
